...
ENTITIES_EMB_FILE = '../data/embedding/TransE_TransE_EPOCH100_EMB100_LR0.1_NEG128_RC1e-06_GAMMA12_ADV0_0/TransE_EPOCH100_EMB100_LR0.1_NEG128_RC1e-06_GAMMA12_ADV0_TransE_entity.npy'
RELATIONS_EMB_FILE = '../data/embedding/TransE_TransE_EPOCH100_EMB100_LR0.1_NEG128_RC1e-06_GAMMA12_ADV0_0/TransE_EPOCH100_EMB100_LR0.1_NEG128_RC1e-06_GAMMA12_ADV0_TransE_relation.npy'
...
X_train, Y_train  = get_labeled_dataset("../dataset/scicite_origin/train.txt",e2id,r2id)
X_dev, Y_dev = get_labeled_dataset("../dataset/scicite_origin/valid.txt",e2id,r2id)
...
class MLP(torch.nn.Module):
    def __init__(self,dropout):
        ...
        self.linear1 = torch.nn.Linear(100*2, 64)
...
for DROPOUT in [0.0,0.2,0.4]:
    Basic_linear_model = MLP(DROPOUT).to(device)
    optimizer = torch.optim.Adam(Basic_linear_model.parameters())
    loss_fn  = torch.nn.CrossEntropyLoss()
    test_max_f1 = 0
    test_max_epoch = 0
    for i in range(100):
        ...
        test_loss,y_true,y_predict = evaluation(Basic_linear_model,test_generator,loss_fn)
        if (i+1)%10==0:
            print('Epoch: {} [Train] Loss: {:.4f} A: {:.2f} [Dev] Loss: {:.4f} A: {:.2f} P: {:.2f} R: {:.2f} F1: {:.2f} [Test] Loss: {:.4f} A: {:.2f} P: {:.2f} R: {:.2f} F1: {:.2f}'.format(            
                i+1, 
                train_loss,float(train_acc)*100,
                dev_loss,
                accuracy_score(y_true_dev,y_predict_dev)*100,
                precision_score(y_true_dev,y_predict_dev,average="macro")*100,
                recall_score(y_true_dev,y_predict_dev,average="macro")*100,
